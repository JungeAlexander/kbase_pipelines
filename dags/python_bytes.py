...
def get_recent_episodes(recent_episodes_date: DateTime) -> Dict:
    ...
    for item in root.iter("item"):
        ...
        id_ = (
            "PythonBytes:"
            + tag_to_text["{http://www.itunes.com/dtds/podcast-1.0.dtd}episode"].strip()
        )
        assert len(id_) > 0
        ...
        if submit_requests:
            r = requests.get(
                f"{api_url}/documents/{id_}", headers=superuser_token_headers
            )
...
def remove_existing_episodes(recent_episodes: Dict, token_headers: Dict) -> Dict:
    return {}
...
def python_bytes_dag():
    ...
    def get_new_episodes(**kwargs) -> Dict:
        ...
        new_episodes = remove_existing_episodes(recent_episodes, token_headers)
        return new_episodes
